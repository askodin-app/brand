...
// Standard virtual background: 1920x1080 (16:9)
const W = 1920;
const H = 1080;
...
function generateBackground() {
  ...
  // Bottom accent bar — solid green
  elements.push(`<rect y="${H - 4}" width="${W}" height="4" fill="${GREEN}"/>`);

  // Ambient glow zones — visible color washes
  elements.push(`<ellipse cx="${W * 0.25}" cy="${H * 0.4}" rx="550" ry="450" fill="url(#glowGreen)"/>`);
  elements.push(`<ellipse cx="${W * 0.75}" cy="${H * 0.6}" rx="500" ry="400" fill="url(#glowOrange)"/>`);

  // Infrastructure grid — visible but not distracting
  ...
  // Grid intersection dots — adds data-point texture
  for (let x = 120; x < W; x += 120) {
    for (let y = 80; y < H; y += 80) {
      // Skip center where face sits
      const dx = x - W / 2;
      const dy = y - H / 2;
  ...
  // Diamond accents — symmetric for mirror-safety, clearly visible
  const diamonds = [
    // Outer frame — large diamonds
    { x: 120, y: 120, size: 14, color: ORANGE, opacity: 0.35 },
    { x: 1800, y: 120, size: 14, color: ORANGE, opacity: 0.35 },
    { x: 120, y: 960, size: 14, color: ORANGE, opacity: 0.35 },
    { x: 1800, y: 960, size: 14, color: ORANGE, opacity: 0.35 },

    // Secondary frame
    { x: 300, y: 80, size: 10, color: GREEN, opacity: 0.4 },
    { x: 1620, y: 80, size: 10, color: GREEN, opacity: 0.4 },
    { x: 300, y: 1000, size: 10, color: GREEN, opacity: 0.4 },
    { x: 1620, y: 1000, size: 10, color: GREEN, opacity: 0.4 },

    // Mid sides
    { x: 100, y: 540, size: 10, color: GREEN, opacity: 0.3 },
    { x: 1820, y: 540, size: 10, color: GREEN, opacity: 0.3 },

    // Inner ring
    { x: 480, y: 160, size: 8, color: ORANGE, opacity: 0.25 },
    { x: 1440, y: 160, size: 8, color: ORANGE, opacity: 0.25 },
    { x: 480, y: 920, size: 8, color: ORANGE, opacity: 0.25 },
    { x: 1440, y: 920, size: 8, color: ORANGE, opacity: 0.25 },

    { x: 360, y: 300, size: 6, color: GREEN, opacity: 0.2 },
    { x: 1560, y: 300, size: 6, color: GREEN, opacity: 0.2 },
    { x: 360, y: 780, size: 6, color: GREEN, opacity: 0.2 },
    { x: 1560, y: 780, size: 6, color: GREEN, opacity: 0.2 },

    // Small accents near edges
    { x: 200, y: 400, size: 5, color: ORANGE, opacity: 0.2 },
    { x: 1720, y: 400, size: 5, color: ORANGE, opacity: 0.2 },
    { x: 200, y: 680, size: 5, color: ORANGE, opacity: 0.2 },
    { x: 1720, y: 680, size: 5, color: ORANGE, opacity: 0.2 },

    { x: 600, y: 100, size: 5, color: GREEN, opacity: 0.18 },
    { x: 1320, y: 100, size: 5, color: GREEN, opacity: 0.18 },
    { x: 600, y: 980, size: 5, color: GREEN, opacity: 0.18 },
    { x: 1320, y: 980, size: 5, color: GREEN, opacity: 0.18 },
  ];
  ...
  // Corner brackets — clearly visible framing
  const bracketSize = 60;
  const bracketOffset = 40;
  const bracketStroke = 2.5;
  const bracketOpacity = 0.3;
  ...
  // Concentric circles at center — visible but don't compete with face
  for (let r = 120; r <= 400; r += 70) {
    const op = (0.08 - (r / 400) * 0.05).toFixed(3);
    elements.push(`<circle cx="${W / 2}" cy="${H / 2}" r="${r}" fill="none" stroke="${GREEN}" stroke-width="1.5" opacity="${op}"/>`);
  }

  const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}">
  <title>askOdin Virtual Background</title>
  ${elements.join('\n  ')}
</svg>`;

  fs.writeFileSync(path.join(OUTPUT_DIR, 'askOdin-virtual-bg.svg'), svg);
  console.log('Virtual background SVG generated');

  return svg;
}

async function main() {
  console.log('=== Virtual Background Generator (1920 x 1080) ===\n');
  generateBackground();

  const svgBuf = fs.readFileSync(path.join(OUTPUT_DIR, 'askOdin-virtual-bg.svg'));

  // Standard 1080p
  await sharp(svgBuf, { density: 150 })
    .resize(1920, 1080, { fit: 'fill' })
    .png()
    .toFile(path.join(OUTPUT_DIR, 'askOdin-virtual-bg.png'));
  console.log('Virtual background PNG generated (1920x1080)');

  // 720p for lower bandwidth
  await sharp(svgBuf, { density: 150 })
    .resize(1280, 720, { fit: 'fill' })
    .png()
    .toFile(path.join(OUTPUT_DIR, 'askOdin-virtual-bg-720p.png'));
  console.log('Virtual background PNG 720p generated (1280x720)');
}
