...
// One folder per publication, dated by the article's pubDate (2026-09-09) and
// named with its URL slug. Deliverable PNGs sit in banners/; source SVGs in svg/.
const ASSET_DIR = './output/social/20260909-information-is-free-judgment-is-unscalable';
...
// ---------------------------------------------------------------------------
// Copy — https://askodin.app/insights/information-is-free-judgment-is-unscalable/
// ---------------------------------------------------------------------------
// The published title is "Information Is Free. Judgment Is the Last Unscalable
// Asset." — too long to set as display type and still survive a phone share
// card. The card takes the frontmatter `seoTitle`, which is the same claim in
// two beats and is already the house's own short form of it.
const HEADLINE = ['Information Is Free.', 'Judgment Is Unscalable.'];
...
const HEADLINE_TALL = ['Information', 'Is Free.', 'Judgment Is', 'Unscalable.'];

// Matches the published subtitle in askodin-coming-soon's frontmatter — keep
// these in sync if the article is edited. Too long for one line at display
// scale, so it is pre-broken rather than left to shrink to footnote size.
const STANDFIRST = [
  'Why institutional capital allocation requires deterministic',
  'AI Judgment Infrastructure — not probabilistic summarization.',
];
const STANDFIRST_TALL = [
  'Why institutional capital allocation requires',
  'deterministic AI Judgment Infrastructure',
...
async function main() {
  console.log('=== Information Is Free. Judgment Is Unscalable. ===\n');

  const TITLE = 'Information Is Free. Judgment Is Unscalable. — askOdin';
  ...
  await emit('askOdin-information-is-free-og', 1200, 630,
    svgFor(TITLE, 1200, 630, card(1200, 630, { s: 1, margin: 100 })));

  // Substack hero: shown uncropped in the post body.
  await emit('askOdin-substack-information-is-free-hero', 1600, 900,
  ...
  await emit('askOdin-substack-information-is-free-og', 1456, 1048,
  ...
  await emit('askOdin-information-is-free-x-mobile', 1080, 1350,
  ...
  await emit('askOdin-information-is-free-linkedin-square', 1200, 1200,
