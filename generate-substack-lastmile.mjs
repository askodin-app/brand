...
// Copy
// ---------------------------------------------------------------------------
const HEADLINE = 'The Last Mile of AI';
const STANDFIRST = ['Judgment Infrastructure', 'Defensible Audit Logs', 'The End of Retrieval'];
const FOOTER = 'SSRN PAPER · 2026';
...
async function main() {
  console.log('=== Substack — The Last Mile of AI (SSRN paper) ===\n');

  // Hero: shown uncropped in the post body, so the box fills the frame.
  await emit('askOdin-substack-lastmile-hero', 1600, 900,
    svgFor('The Last Mile of AI — askOdin', 1600, 900, card(1600, 900, 1600)));

  // Social/OG: Substack's recommended share frame. The box sits dead centre,
  // leaving ~175px of clear ground top and bottom for the platforms to crop.
  await emit('askOdin-substack-lastmile-og', 1456, 1048,
    svgFor('The Last Mile of AI — askOdin', 1456, 1048, card(1456, 1048, 1320)));
}
